...
export const getFolderContents = query({
  ...
  handler: async (ctx, args) => {
    ...
    return files.sort((a, b) => {
      ...
      if (a.type === "folder" && b.type === "file") return -1; // -1 means no swap
      if (a.type === "file" && b.type === "folder") return 1; // 1 means swap
...
export const deleteFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const deleteRecursive = async (fileId: typeof args.id) => {
      ...
      // If it's a folder delete all chidren
      if (item.type === "folder") {
      ...
      // Delete storage files if its exists
      if (item.storageId) {
